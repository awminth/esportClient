...
<body>
    <div id="app">
        ...
        <div class="iframe-container" :aria-busy="loading.toString()" role="region"
            aria-label="Embedded content iframe">
            <iframe v-if="iframeUrl" :src="iframeUrl" @load="onIframeLoad" :style="{ opacity: loading ? 0.3 : 1 }"
                sandbox="allow-same-origin allow-scripts allow-forms allow-popups" title="Embedded content"></iframe>
    ...
    createApp({
        ...
        mounted() {
            const encodedUrl = this.getQueryParam('target_url');
            if (encodedUrl) {
                // ❌ decode မလုပ်ပါနဲ့ – already decoded ဖြစ်နေပါတယ်
                this.iframeUrl = '/iframe-proxy.php?url=' + encodeURIComponent(encodedUrl);
            } else {
                this.loading = false;
            }
        }
